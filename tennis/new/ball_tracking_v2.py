...
def fnd_endframe(counter, init_frame, frame,fps, border, \
...
##--End preparation-------------------------------------------------------------------------------------------------------------
 ...
 ##We find all contours admissible size, shape and color plus it shuold be
    ...
    for c in cnts:
        ...
        a1 = box[0]-box[1]
        a2 = box[1]-box[2]
        side_ratio = np.linalg.norm(a1)/(np.linalg.norm(a2)+0.0000000001)
        box = np.int0(box)
